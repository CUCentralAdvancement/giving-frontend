...
export default function Checkout() {
  ...
  function submitHandler(data) {
    ...
    dispatch({ type: 'add_giving_info', payload: data });
    ...
    fetch('/api/authorize-token', {
      ...
      .then((data) => {
        if (data.token) {
          dispatch({ type: 'add_auth_token', payload: data.token });
  ...
  return (
    <Layout>
      <div className='flex flex-col max-w-screen-lg mx-auto p-4'>
        ...
        <hr />
        <CartSummary cart={user.giftBasket} removeCallback={null} />
