...
export default function Checkout() {
  const { addAuthToken, addGivingInfo, giftBasket } = useDonor();
  ...
  function submitHandler(data) {
    const payload = giftBasket.reduce(
      (acc, item) => ({
        ...acc,
        description: acc.description + item.fund_title + ' (' + item.allocation_code + ')',
        orderTotal: parseFloat(acc.orderTotal) + parseFloat(`${item['giving-amount']}.00`),
      }),
      { description: '', orderTotal: 0.0, invoiceNumber: uuidv4().slice(0, 8) },
    );

    addGivingInfo(payload);
  ...
  return (
    <Layout>
      <div className="flex flex-col max-w-screen-lg mx-auto p-4">
        ...
        <hr />
        <CartSummary cart={giftBasket} removeCallback={null} />
